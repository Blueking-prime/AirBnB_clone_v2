...
class test_DBStorage(unittest.TestCase):
    ...
    def testState(self):
        state = State(name="Simret")
        if state.id in models.storage.all():
            self.assertTrue(state.name, "Simret")

    def testCity(self):
        city = City(name="NCity")
        if city.id in models.storage.all():
            self.assertTrue(city.name, "NCity")

    def testPlace(self):
        place = Place(name="Best apartment", number_rooms=5)
        if place.id in models.storage.all():
            self.assertTrue(place.number_rooms, 5)
            self.assertTrue(place.name, "Best apartment")
    ...
    def testReview(self):
        ...
        if review.id in models.storage.all():
            self.assertTrue(review.text, "Whazup")
